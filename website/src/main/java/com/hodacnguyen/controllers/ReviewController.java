...
public class ReviewController {
    ...
    public String addreview(Model model ,@AuthenticationPrincipal User user,@PathVariable(name = "idbilltype") int idbilltype,@RequestParam(name = "comment")String comment, @RequestParam(name = "rating")int rating,@RequestParam("img[]") MultipartFile[] files,HttpServletRequest req){
        Map<String,Object> map = new HashMap<>();
        map.put("tags", tagService.list(0, 8));
        com.hodacnguyen.pojo.User t= userService.getUsers(user.getUsername()).get(0);
        BillType billType =billTypeService.getById(idbilltype);
